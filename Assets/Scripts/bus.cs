...
public class bus : MonoBehaviour{
    ...
    {   
        ...
        */
        ...
        if (!pause){
            ...
            if (speed != 0 && Time.time - timecheck >= 0.15) {
                timecheck = Time.time;
    ...
    void changeSpeed(){ //속도변화
        ...
        velocity += 0.0005f * mouse.crossP;

        if (velocity >= 1) { // 속도가 증가하기 위한 최소한의 마우스 회전속도
            speed += accel;
            
            // 마우스 회전 속도에 따른 추가속도 더하기
            if (velocity >= 10) speed += velocity * 0.001f;
            velocity = 0;
            timecheck = Time.time;
        }
        else if (velocity <= -1){
    ...
    void rotateWheel(int wheel, string s){
        ...
        // 버스가 움직일때 굴러가는 바퀴
        if (Math.Abs(speed) <= 2) tires[wheel].Rotate(Vector3.right * -speed * Time.deltaTime * 20);
        else if (Math.Abs(speed) <= 3) tires[wheel].Rotate(Vector3.right * -speed * Time.deltaTime * 18);
        else if (speed < 0) tires[wheel].Rotate(Vector3.right * Time.deltaTime * 54);
        else if (speed > 0) tires[wheel].Rotate(Vector3.right * -Time.deltaTime * 54);
        // 앞바퀴 좌우 회전
        switch(s){
            /*
